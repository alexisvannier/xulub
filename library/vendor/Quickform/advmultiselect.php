...
* @version    CVS: $Id: advmultiselect.php,v 1.7 2005/08/27 13:17:13 farell Exp $
...
';
    ...
    function toHtml()
    {
        ...
        if (strpos($this->_elementTemplate, '{unselected}') === false) {
            ...
            $strHtmlSelected = $tab;
            $aff_div = false;
            // Si on a qu'un seul élément, pas de div
            if ( count($this->_options) > 1 )
            {   
            	$aff_div = true;         
            	$strHtmlSelected .= '<div id="'.$this->getName().'">';
            }
            $strHtmlSelected .= PHP_EOL;
            ...
            foreach ($this->_options as $option) {
                ...
                if ( $aff_div ) 
                {
                	$strHtmlSelected .= '<label '
                						. $this->_getAttrString($labelAttributes) .'>';
                }

                $strHtmlSelected .=  '<input type="checkbox"'
                                 .  ' name="'.$selectName.'"'
                                 .  $checked
